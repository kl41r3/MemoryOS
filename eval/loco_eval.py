...
def calculate_nlp_metrics(gold_answer, response, options=None):
    if options is None:
    ...
    if "semantic" in options:
        ...
        try:
            ...
            if f1 is not None:
                if hasattr(f1, 'item'):
                    metrics["semantic"]["bert_f1"] = float(f1.item())
                elif hasattr(f1, 'tolist'):
                    metrics["semantic"]["bert_f1"] = float(f1.tolist()[0])
                else:
                    metrics["semantic"]["bert_f1"] = float(f1)
            else:
                metrics["semantic"]["bert_f1"] = 0.0
        except Exception as e:
            print(f"Warning: bert_score calculation failed: {e}")
            metrics["semantic"]["bert_f1"] = 0.0
...
async def process_group_responses(group_id, group_responses, oai_client, options, num_runs: int):
    ...
    for response in tqdm(group_responses, desc=f"Processing group {group_id}"):
        ...
        answer = response.get("system_answer")
        ground_truth = response.get("original_answer")
        ...
        nlp_metrics = calculate_nlp_metrics(ground_truth, answer, options)
...
async def main(frame, version="default", options=None, num_runs=1, max_workers=4):
    ...
    response_path = "all_loco_results.json"
    judged_path = "all_loco_judged.json"
    ...
    # Group responses by sample_id
    grouped_responses = {}
    for response in locomo_responses:
        sample_id = response.get("sample_id", "unknown")
        if sample_id not in grouped_responses:
            grouped_responses[sample_id] = []
        grouped_responses[sample_id].append(response)

    all_grades = {}
    total_responses_count = len(locomo_responses)
    num_groups = len(grouped_responses)
    
    print(f"Found {total_responses_count} total responses across {num_groups} conversation groups to evaluate")
    print(f"Groups: {list(grouped_responses.keys())}")
    ...
    print(f"Starting evaluation of {active_groups} conversation groups with responses")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lib",
        type=str,
        choices=["memoryos", "zep", "memos", "mem0", "mem0_graph", "openai", "memos-api", "memobase"],
    )
    parser.add_argument(
        "--version",
        type=str,
